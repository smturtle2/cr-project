"""Ablation: CrossModalBlock ×1 (body1 뒤만) — ×2 대비 baseline.

모델:  ACA_CRNet(use_cafm=False, use_cross_modal=True, cross_modal_num_blocks=1)
       - CrossModalBlock을 body1 이후 1곳에만 삽입
Optim: AdamW(lr=1e-4)
Loss:  MAE (L1Loss)
...
def main() -> None:
    with use_local_builds():
        shared_main.main(
            ...
            val_max_samples=64,
            test_max_samples=64,
            output_dir=Path("artifacts/module1_mae_xmodal1_20ep"),
            resume=None,
            run_test=True,
            num_examples=5,
            example_mode="best",
