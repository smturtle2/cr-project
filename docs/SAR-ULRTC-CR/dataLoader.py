...
class SEN12MSCR_new0(Dataset):
    ...
    def __getitem__(self, pdx):  # get the triplet of patch with ID pdx
        ...
        sample = {'input': {'S1': s1,
                            'S2': process_MS(s2_cloudy, self.method),
                            'masks': mask if mask is not None else np.zeros(s2_cloudy.shape[1:]),
                            'coverage': np.mean(mask) if mask is not None else 0,
                            'S1 path': os.path.join(self.root_dir, self.paths[pdx]['S1']),
                            'S2 path': os.path.join(self.root_dir, self.paths[pdx]['S2_cloudy']),
                            'coord': coord,
                            },
...
class SEN12MSCRTS_2(Dataset):
    ...
    def __getitem__(self, pdx):  # get the time series of one patch
        ...
        # s1              = [process_SAR(read_img(img), self.method) for img in s1_tif]
        s2              = [read_img(img) for img in s2_tif]  # note: pre-processing happens after cloud detection
        # print(self.paths[pdx]['S2'])
        masks           = None if not self.cloud_masks else [get_cloud_map(img, self.cloud_masks, self.cloud_detector) for img in s2]
        # masks = [np.ones_like(img, dtype=float) for img in s2]
        # get statistics and additional meta information
        if masks != None:
            coverage    = [np.mean(mask) for mask in masks]
        else:
            coverage    = [0.0] * len(s2)
            #print(len(s2), len(coverage),111111111)
        # s1_dates    = [to_date(img.split('/')[-1].split('_')[5]) for img in self.paths[pdx]['S1']]
        s2_dates    = [to_date(img.split('/')[-1].split('_')[5]) for img in self.paths[pdx]['S2']]
        # s1_td       = [(date-S1_LAUNCH).days for date in s1_dates]
        s2_td       = [(date-S1_LAUNCH).days for date in s2_dates]
        ...
        if self.sample_type == 'cloudy_cloudfree':
            if self.import_data_path:
                ...
                # target_s1, target_s2, target_mask = np.array(s1)[cloudless_idx], np.array(s2)[cloudless_idx], np.array(masks)[cloudless_idx]
                # input_s1, input_s2, input_masks   = np.array(s1)[inputs_idx], np.array(s2)[inputs_idx], np.array(masks)[inputs_idx]
                target_s2, target_mask = np.array(s2)[cloudless_idx], np.array(masks)[cloudless_idx]
                input_s2, input_masks   = np.array(s2)[inputs_idx], np.array(masks)[inputs_idx]
                coverage_match = True
                ...
                if self.export_data_path:
                    ...
                    self.data_pairs[pdx] = {'input': inputs_idx, 'target': cloudless_idx,
                                            ...
                                            #          'output': {'S1': self.paths[pdx]['S1'][cloudless_idx],
                                            #                     'S2': self.paths[pdx]['S2'][cloudless_idx]}}}
                                            'paths': {'input': {'S2': [self.paths[pdx]['S2'][idx] for idx in inputs_idx]},
                                                      'output': {'S2': self.paths[pdx]['S2'][cloudless_idx]}}}
                    if os.path.isdir(self.export_data_path):
                        export_here = os.path.join(self.export_data_path, f'{self.n_input_t}_{self.split}_{self.cloud_masks}.npy')
                    else:
                        export_here = self.export_data_path
            ...
            # sample = {'input': {'S1': list(input_s1),
            #                     'S2': [process_MS(img, self.method) for img in input_s2],
            sample = {'input': {'S2': [process_MS(img, self.method) for img in input_s2],
                                'masks': list(input_masks),
                                'coverage': [np.mean(mask) for mask in input_masks],
                                # 'S1 TD': [s1_td[idx] for idx in inputs_idx],
                                'S2 TD': [s2_td[idx] for idx in inputs_idx],
                    ...
                    #   'target': {'S1': [target_s1],
                      'target': {'S2': [process_MS(target_s2, self.method)],
                                 'masks': [target_mask],
                                 'coverage': [np.mean(target_mask)],
                                #  'S1 TD': [s1_td[cloudless_idx]],
                                 'S2 TD': [s2_td[cloudless_idx]],
                                #  'S1 path': [os.path.join(self.root_dir, self.paths[pdx]['S1'][cloudless_idx])],
                                 'S2 path': [os.path.join(self.root_dir, self.paths[pdx]['S2'][cloudless_idx])],
                                 'coord': [coord[cloudless_idx]],
                                 'cloudless_idx': [cloudless_idx],
                                 },
                       'coverage bin': coverage_match
                      }
            print('Got cloudy_cloudfree sample!')

        elif self.sample_type == 'generic':  # this returns the whole, unfiltered sequence of S1 & S2 observations
            sample = {'S2': [process_MS(img, self.method) for img in s2],
                      'masks': masks,
                      'coverage': coverage,
                      'S2 TD': s2_td,
                      'S2 path': [os.path.join(self.root_dir, self.paths[pdx]['S2'][idx]) for idx in self.time_points],
...
class SEN12MSCRTS_3(Dataset):
    def __init__(self, root, split="all", region='all', cloud_masks='s2cloudless_mask', sample_type='cloudy_cloudfree', n_input_samples=3, rescale_method='default', min_cov=0.0, max_cov=1.0, import_data_path=None, export_data_path=None):
        ...
        self.export_data_path = export_data_path
        if self.export_data_path: self.data_pairs = {}
        # print(self.import_data_path)
        if self.import_data_path:
            
            # fetch time points as specified in the imported file, expects arguments are set accordingly
            if os.path.isdir(self.import_data_path):
                import_here = os.path.join(self.import_data_path, f'{self.n_input_t}_{self.split}_{self.region}_{self.cloud_masks}.npy')
            else:
                import_here = self.import_data_path
            self.data_pairs = np.load(import_here, allow_pickle=True).item()
            print(f'Importing data pairings for split {self.split} from {import_here}.')
    ...
    def __getitem__(self, pdx):  # get the time series of one patch
        ...
        s2              = [read_img(img) for img in s2_tif]  # note: pre-processing happens after cloud detection
        # masks           = None if not self.cloud_masks else [get_cloud_map(img, self.cloud_masks, self.cloud_detector) for img in s2]
        masks           = [np.ones((img.shape[-1], img.shape[-1])) for img in s2]
                    ...
                    self.data_pairs[pdx] = {'input': inputs_idx, 'target': cloudless_idx,
                                            'paths': {'input': {'S1': [self.paths[pdx]['S1'][idx] for idx in inputs_idx],
                                                      ...
                                                      'output': {'S1': self.paths[pdx]['S1'][cloudless_idx],
                                                                 'S2': self.paths[pdx]['S2'][cloudless_idx]}}}
                    if os.path.isdir(self.export_data_path):
                        export_here = os.path.join(self.export_data_path, f'{self.n_input_t}_{self.split}_{self.region}_{self.cloud_masks}.npy')
                    else:
                        export_here = self.export_data_path
                    np.save(export_here, self.data_pairs)
...
class SEN12MSCR_2(Dataset):
    ...
    def __getitem__(self, pdx):  # get the triplet of patch with ID pdx
        ...
        s2_cloudy       = read_img(s2_cloudy_tif)    # note: pre-processing happens after cloud detection
        # mask            = None if not self.cloud_masks else get_cloud_map(s2_cloudy, self.cloud_masks, self.cloud_detector)
        mask            = np.ones((s2.shape[-1], s2.shape[-1]))

        sample = {'input': {'S1': s1,
                            'S2': process_MS2(s2_cloudy, self.method),
                            'masks': mask,
                            'coverage': np.mean(mask),
                            'S1 path': os.path.join(self.root_dir, self.paths[pdx]['S1']),
                            'S2 path': os.path.join(self.root_dir, self.paths[pdx]['S2_cloudy']),
                    ...
                    'target': {'S2': process_MS2(s2, self.method),
                               'S2 path': os.path.join(self.root_dir, self.paths[pdx]['S2']),
                               'coord': coord,
                                },
                    }
...
class SEN12MSCR_new(Dataset):
    # SEN12MSCR_new is a new version of SEN12MSCR dataset, which is used for the new model set up
    ...
    # |   |...
    # |   ├─patchN.pt
    # The dataset is split into train, test, and val sets
    # To use the dataset, the user needs to specify the root directory and the split type
    # The dataset will return a dictionary containing the input and target images
    # Input images are stored in the 'input' key, and target images are stored in the 'target' key
    # For example, sample = {'input': input_img, 'target': target_img}
    # input_img = sample['input']
    # target_img = sample['target']
    
    def __init__(self, root, split="all", rescale_method='default', data_mode = 'partial'):
        self.root_dir = root
        self.split = split
        ...
        if self.split == 'train':
            if self.data_mode == 'full':
                self.input_dir = os.path.join(self.root_dir, 'train_masked_full_npy')
                self.target_dir = os.path.join(self.root_dir, 'train_gt_full_npy')
                self.sar_dir = os.path.join(self.root_dir, 'train_sar_full_npy')
            elif self.data_mode == 'partial':
                self.input_dir = os.path.join(self.root_dir, 'train_masked_npy')
                self.target_dir = os.path.join(self.root_dir, 'train_gt_npy')
                self.sar_dir = os.path.join(self.root_dir, 'train_sar_npy')
                self.trans_dir = os.path.join(self.root_dir, 'train_translated')
            else:
                raise ValueError("Invalid data_mode. Choose either 'partial' or 'full'.")
        elif self.split == 'test':
            if self.data_mode == 'full':
                self.input_dir = os.path.join(self.root_dir, 'test_masked_full_npy')
                self.target_dir = os.path.join(self.root_dir, 'test_gt_full_npy')
                self.sar_dir = os.path.join(self.root_dir, 'test_sar_full_npy')
                # self.trans_dir = os.path.join(self.root_dir, 'test_translated')
            elif self.data_mode == 'partial':
                self.input_dir = os.path.join(self.root_dir, 'test_masked_100_npy')
                self.target_dir = os.path.join(self.root_dir, 'test_gt_100_npy')
                self.trans_dir = os.path.join(self.root_dir, 'test_translated')
                self.sar_dir = os.path.join(self.root_dir, 'test_sar_100_npy')
            else:
                raise ValueError("Invalid data_mode. Choose either 'partial' or 'full'.")
        elif self.split == 'val':
            self.input_dir = os.path.join(self.root_dir, 'val_masked_npy')
            self.target_dir = os.path.join(self.root_dir, 'val_gt_npy')
            self.sar_dir = os.path.join(self.root_dir, 'val_sar_npy')
            self.trans_dir = os.path.join(self.root_dir, 'val_translated')
        else:
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            filename = os.listdir(self.input_dir)[idx]
            input_path = os.path.join(self.input_dir, filename)
            target_path = os.path.join(self.target_dir, filename)
            sar_path = os.path.join(self.sar_dir, filename)
            trans_path = os.path.join(self.trans_dir, filename.replace('.npy', '.png'))
            input_img = torch.from_numpy(np.load(input_path).astype('float32'))
            target_img = torch.from_numpy(np.load(target_path).astype('float32'))
            sar_img = torch.from_numpy(np.load(sar_path).astype('float32'))
            trans_img = cv2.imread(trans_path, cv2.IMREAD_COLOR)
            if trans_img is None:
                raise FileNotFoundError(f"Image not found at path: {trans_path}")
            trans_img = cv2.cvtColor(trans_img, cv2.COLOR_BGR2RGB)
            trans_img = np.array(trans_img) / 255.0
            trans_img = torch.from_numpy(trans_img.transpose(2, 0, 1).astype('float32'))
            sample = {'input': input_img, 'target': target_img, 'sar': sar_img, 'trans': trans_img, 'filename': filename}
            # sample = {'input': input_img, 'target': target_img, 'sar': sar_img, 'filename': filename}
            return sample
        elif self.split == 'test':
            filename = os.listdir(self.input_dir)[idx]
            input_path = os.path.join(self.input_dir, filename)
    ...
    def load_pt(self, input_path):
        input_img = torch.load(input_path)
        return input_img

class SIM_MSCR(Dataset):
    # The dataset needs to have the following structure:
    # root_dir
    # ├───train_masked_npy
    # |   ├─patch1.npy
    ...
    # The dataset is split into train, test, and val sets.
    # To use the dataset, the user needs to specify the root directory and the split type.
    # The dataset will return a dictionary containing the input, target, sar, and trans images.
    # Input images are stored in the 'input' key, target images are stored in the 'target' key,
    # sar images are stored in the 'sar' key.
    # For example, sample = {'input': input_img, 'target': target_img, 'sar': sar_img, 'filename': filename}
    # input_img = sample['input']
    # target_img = sample['target']
    # sar_img = sample['sar']
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            if trans_img is None:
                raise FileNotFoundError(f"Image not found at path: {trans_path}")
            trans_img = cv2.cvtColor(trans_img, cv2.COLOR_BGR2RGB).astype('float32') / 255.0
    ...
    def load_npy(self, input_path):
        input_img = torch.load(input_path)
        return input_img

class SEN12MSCR_0(Dataset):
    # The dataset needs to have the following structure:
    # root_dir
    # ├───train_masked_npy
    # |   ├─patch1.npy
    ...
    def __init__(self, root, split="all", rescale_method='default', data_mode = 'partial'):
        ...
        if self.split == 'train':
            if self.data_mode == 'full':
                self.input_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_input_npy_full')
                self.target_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_gt_npy_full')
                self.sar_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_sar_npy_full')
                self.mask_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_mask_npy_full')
                self.trans_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_translated_full')
            elif self.data_mode == 'partial':
                self.input_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_input_npy')
                self.target_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_gt_npy')
                self.sar_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_sar_npy')
                self.trans_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_translated')
                self.mask_dir = os.path.join(self.root_dir, 'SEN12MSCR_train_mask_npy')
            else:
                raise ValueError("Invalid data_mode. Choose either 'partial' or 'full'.")
        elif self.split == 'test':
            if self.data_mode == 'full':
                self.input_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_input_npy_full')
                self.target_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_gt_npy_full')
                self.sar_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_sar_npy_full')
                self.trans_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_translated_full')
                self.mask_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_mask_npy_full')
            elif self.data_mode == 'partial':
                self.input_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_input_npy')
                self.target_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_gt_npy')
                self.sar_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_sar_npy')
                self.trans_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_translated')
                self.mask_dir = os.path.join(self.root_dir, 'SEN12MSCR_test_mask_npy')
            else:
                raise ValueError("Invalid data_mode. Choose either 'partial' or 'full'.")
        elif self.split == 'val':
            self.input_dir = os.path.join(self.root_dir, 'SEN12MSCR_val_input_npy')
            self.target_dir = os.path.join(self.root_dir, 'SEN12MSCR_val_gt_npy')
            self.sar_dir = os.path.join(self.root_dir, 'SEN12MSCR_val_sar_npy')
            self.trans_dir = os.path.join(self.root_dir, 'SEN12MSCR_val_translated')
            self.mask_dir = os.path.join(self.root_dir, 'SEN12MSCR_val_mask_npy')
        else:
            raise ValueError("Invalid split type.")
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            mask_img = torch.from_numpy(np.load(mask_path).astype('float32'))
            mask_img = mask_img.unsqueeze(0).repeat(13, 1, 1)
            sample = {'input': input_img, 'target': target_img, 'sar': sar_img, 'trans': trans_img, 'filename': filename, 'mask': mask_img}
            # sample = {'input': input_img, 'target': target_img, 'sar': sar_img, 'filename': filename}
            return sample
        elif self.split == 'test':
            filename = os.listdir(self.input_dir)[idx]
